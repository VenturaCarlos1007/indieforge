...
export default function NotificationBell() {
  ...
  return (
    <div className="relative z-50" ref={panelRef}>
      <Tooltip text="Notificaciones" side="bottom">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="relative p-2 rounded-xl bg-black/[0.04] dark:bg-white/5 hover:bg-black/[0.07] dark:hover:bg-white/10 transition-colors border border-black/[0.07] dark:border-white/[0.06]"
        >
          <Bell className="w-5 h-5 text-surface-400" />
          {unreadCount > 0 && (
            <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse border border-white dark:border-gray-900" />
          )}
        </button>
      </Tooltip>
